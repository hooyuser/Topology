...
\cor[GroupEmbedding]{Fundamental Group Embedding for Covering Spaces}{
	Suppose $p : E \to X$ is a covering of spaces and $e\in E$. Then $p_*: \pi_1(E, e) \to \pi_1(X, x)$ is a monomorphism. The image $p_*(\pi_1(E, e))$ consists of all equivalent classes of loops in $X$ that lift to loops in $E$ based at $e$.
}
\pf{ 
...
\dfn{Reconstructing Functor $\mathrm{Rec}:\left[\Pi_1(X),\mathsf{Set}\right] \to\mathsf{Cov}_X$}{
	...
	\end{align*}
	and check $p$ is actually a covering. The reconstructing functor is defined as
	\begin{equation*}
		\begin{tikzcd}[ampersand replacement=\&]
			\left[\Pi_1(X),\mathsf{Set}\right] \&[-25pt]\&[+10pt]\&[-30pt]\mathsf{Cov}_X\&[-20pt]\&[-30pt] \\ [-15pt] 
			\rho_1  \arrow[dd, "f"{left}, Rightarrow] \& \&  \&  E(\rho_1)\arrow[dd, "\mathrm{Rec}_E(f)"]\&\ni\& (x,e)\arrow[dd,mapsto]\\ [-10pt] 
											\&  \phantom{.}\arrow[r, "\mathrm{Rec}_E", squigarrow]\&\phantom{.}  \&   \\[-10pt] 
			\rho_2 \& \& \& E(\rho_2)\&\ni\& (x, f_x(e))
		\end{tikzcd}
	\end{equation*}
}
...
In particular, if $p$ is a normal covering, $\mathrm{Aut}_{\mathsf{Cov}_X}(p)\cong \pi_1\left(X, x_0\right) / H$. If $p$ is a universal covering, we have $G(\tilde{X}) \approx \pi_1(X)$.
	\end{enumerate}
}
