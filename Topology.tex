...
\dfn{Homotopy Lifting Property}{
	Define the inclusion $\iota_0:Y\to Y\times I,y\mapsto(y,0)$. A map $p: E \rightarrow X$ has the \textbf{homotopy lifting property} for the space $Y$ if the following holds: for each homotopy $f_\bullet: Y \times I \rightarrow X$ and each map $\tilde{f}_0: Y \rightarrow E$ such that $p \circ \tilde{f}_0=h \circ\iota_0$, there exists a homotopy $\tilde{f}_{\bullet}: Y \times I \rightarrow E$ with $p \circ \tilde{f}_{\bullet}=f_{\bullet}$ and $ \tilde{f}_{\bullet} \circ \iota_0=\tilde{f}_0$. In other words, homotopy lifting property means that whenever the outer square of the following diagram commutes, there exists a dashed arrow $\tilde{f}_{\bullet}$ such that the whole diagram commutes.
	\[
	\begin{tikzcd}[ampersand replacement=\&]
		Y \arrow[r, "\tilde{f}_0"] \arrow[d, "\iota_0"', hook]                      \& E \arrow[d, "p"] \\ [+10pt]
		Y\times I \arrow[r, "f_\bullet"'] \arrow[ru, "\tilde{f}_{\bullet}", dashed] \& X               
	\end{tikzcd}
	\]
	We call $\tilde{f}_{\bullet}$ a lifting of $f_{\bullet}$ with initial condition $\tilde{f}_0$. The map $p$ is called a \textbf{fibration} if it has the homotopy lifting property for all spaces.
}
