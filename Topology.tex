...
\prop{Unique Path Lifting}{
	Let $p: E \to X$ be a covering. Suppose $x \in X$ and $e\in p^{-1}(x)$. A path $\gamma: I \to X$ with $\gamma(0)=x$ lifts uniquely to a path $\widetilde{\gamma}: I \to E$ such that $\widetilde{\gamma}(0)=e$ and $p \circ \widetilde{\gamma}=\gamma$. We may use $\widetilde{\gamma}^e$ to denote this unique lifting if we want to emphasize the starting point $e$.
	\[
	\begin{tikzcd}[ampersand replacement=\&]
...
\cor{Covering of Spaces Induces Covering of Groupoids}{
	If $p : E \to X$ is a covering of spaces, then $p_*: \Pi_1(E) \to \Pi_1(X)$ is a covering of groupoid.
}
\pf{
	This a corollary of \Cref{th:Lifting}. Suppose $p : E \to X$ is a covering of spaces and $e\in E$. Given a map $p_*:\mathrm{St}(e)\to\mathrm{St}(p(e))$, we can define
	\begin{align*}
	...
	\[
		p_*\circ q^e([\gamma])=p([\widetilde{\gamma}^e])=[p\circ\widetilde{\gamma}^e]=[\gamma]
	\]
	and
	\[
		q^e\circ p_*([\gamma])=q^e([p\circ\gamma])=[\left(\widetilde{p\circ\gamma}\right)^e]=[\gamma],
	\]
	where $\left(\widetilde{p\circ\gamma}\right)^e=\gamma$ holds since $\gamma$ is a lifting of $p\circ\gamma$ starting at $e$.
}
\prop{}{
	Suppose $p : E \to X$ is a covering of spaces and $e\in E$. Then $p_*: \pi_1(E, e) \to \pi_1(X, x)$ is a monomorphism.
}
\pf{ 
	Let $[\gamma]\in \pi_1(E, e)$ be a loop based at $e$ such that $p_*([\gamma])=[c_x]$. $\gamma$  Since $c_e$ is a lifting of $c_x$ starting at $e$, we have $\gamma=c_e$, which implies $p_*$ is a monomorphism.
}
\dfn{Monodromy Action}{
	...
	\begin{equation*}
		\begin{tikzcd}[ampersand replacement=\&]
			\Pi_1(X)\&[-25pt]\&[+10pt]\&[-30pt]\mathsf{Set}\&[-30pt]\&[-30pt] \\ [-15pt] 
