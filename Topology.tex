...
\begin{proposition}{Equivalent Characterisations of Closed Set}{equivalent_characterisations_of_closed_set}
	Let $X$ be a topological space and $A\subseteq X$. The following propositions are equivalent:
	\begin{enumerate}[(i)]
...
\end{proposition}
\begin{prf}
	(i)\(\implies\)(ii). Let \(x\in A'\). Suppose, for contradiction, that \(x\notin A\). Since \(A\) is closed, \(X-A\) is open. Hence \(X-A\) is an open neighborhood of \(x\). But
	\[
		(X-A)\cap A=\varnothing,
	\]
	so in particular
	\[
		(X-A)\cap (A-\{x\})=\varnothing.
	\]
	This contradicts the fact that $x$ is a limit point of $A$. Therefore \(x\in A\). Hence $A'\subseteq A$.
	
	(ii)\(\implies\)(iii). Suppose \(A'\subseteq A\). We prove that $\overline A\subseteq A$. Let \(x\in \overline A\). Suppose, for contradiction, that \(x\notin A\). Since \(x\in\overline A\), every neighborhood of \(x\) in \(X\) intersects \(A\). But \(x\notin A\), so $A=A-\{x\}$. Hence every neighborhood of \(x\) intersects \(A-\{x\}\). Therefore \(x\in A'\). By assumption \(A'\subseteq A\), so \(x\in A\), a contradiction. 

	(iii)\(\implies\)(i). Suppose \(A=\overline A\). Since \(\overline A\) is closed in \(X\), it follows that \(A\) is closed in \(X\).
\end{prf}
...
\begin{proposition}{Equivalent Definition of Continuous Map}{continuous_map_equivalent_definition}
	...
	\begin{enumerate}[(i)]
		...
		\item For each element $S$ in a subbasis $\mathcal{S}$ of $Y$, $f^{-1}(S)\in\tau_X$.
		\item For any $x\in X$ and for any neighborhood $N\in\mathcal{N}(f(x))$, there exists $M\in\mathcal{N}(x)$ such that $f(M)\subseteq N$.
		\item For every $x \in X$ and every basic neighborhood $V$ of $f(x)$, there is a basic neighborhood $U$ of $x$ with $f(U) \subseteq V$.
...
\begin{definition}{Coarser Topology and Finer Topology}{}
	Let $\tau_1$ and $\tau_2$ be two topologies on a set $X$. If $\tau_1\subseteq\tau_2$, then $\tau_1$ is said to be \textbf{coarser} than $\tau_2$, and $\tau_2$ is said to be \textbf{finer} than $\tau_1$.
\end{definition}

\begin{proposition}{Comparison of the Topologies}{comparison_of_the_topologies}
	Let $\tau_1$ and $\tau_2$ be two topologies on a set $X$. For any $x\in X$, let $\mathcal{B}_1(x)$ and $\mathcal{B}_2(x)$ be neighborhood bases of $x$ with respect to $\tau_1$ and $\tau_2$ respectively.
	\begin{enumerate}[(i)]
		\item $\tau_1 \subseteq \tau_2$.
		\item The identity map $\mathrm{id}: (X, \tau_2) \to (X, \tau_1)$ is continuous.
		\item For any $x \in X$ and any $U_1 \in \mathcal{B}_1(x)$, there exists $U_2 \in \mathcal{B}_2(x)$ such that $U_2 \subseteq U_1$.
	\end{enumerate}
\end{proposition}
...
\subsection{Topology of Pointwise Convergence}


\begin{definition}{Topology of Pointwise Convergence}
{topology_of_pointwise_convergence}
	...
	\begin{itemize}
		...
		\[
		\left( \mathrm{ev}_x|_{A}:A\longrightarrow Y \right)_{x\in X}.
		\]
	\end{itemize}
\end{definition}
\begin{remark}
...
\begin{definition}{Compact-open Topology}{compact_open_topology}
	...
	\[
		S(K,U):=\{f\in\mathrm{Hom}_{\mathsf{Top}}(X,Y)\mid f(K)\subseteq U\}.
	\]
	The \textbf{compact-open topology} on $\mathrm{Hom}_{\mathsf{Top}}(X,Y)$ is the topology generated by the subbasis 
	\[
		\mathcal{S}:=\{S(K,U)\mid K\subseteq X \text{ is compact},\; U\subseteq Y \text{ is open}\}.
	\]
\end{definition}

\begin{proposition}{Compact-open Topology from Discrete Space}{}
	If $X$ is a discrete space and $Y$ is a topological space, then the compact-open topology on $\mathrm{Hom}_{\mathsf{Top}}(X,Y)=\mathrm{Hom}_{\mathsf{Set}}(X,Y)$ coincides with the \hyperref[th:topology_of_pointwise_convergence]{topology of pointwise convergence}.
\end{proposition}
\begin{proof}
	...
	\[
		S(K,U)=\{f\in\mathrm{Hom}_{\mathsf{Set}}(X,Y)\mid f(K)\subseteq U\}=\bigcap_{x\in K} \{f\in\mathrm{Hom}_{\mathsf{Set}}(X,Y)\mid f(x)\in U\}=\bigcap_{x\in K} P(x,U).
	\]
	Therefore the topologies generated by all $P(x,U)$ and all $W(K,U)$ are the same.
\end{proof}


\begin{proposition}{Compact-open Topology from Compact Space to Metric Space}{}
	Let $X$ be a compact topological space, and $(Y,d)$ is a metric space, then the compact-open topology on $\mathrm{Hom}_{\mathsf{Top}}(X,Y)$ is metrizable, and a metric for it is given by
	\begin{align*}
		e: \mathrm{Hom}_{\mathsf{Top}}(X,Y)\times \mathrm{Hom}_{\mathsf{Top}}(X,Y) &\longrightarrow \mathbb{R}_{\ge 0} \\
		(f,g) &\longmapsto \sup_{x \in X} d\left(f(x),g(x)\right)
	\end{align*}
\end{proposition}
\begin{proof}
	Since $x\mapsto d(f(x),g(x))$ is a continuous function from $X$ to $\mathbb{R}_{\ge 0}$, and $X$ is compact, $e(f,g)$ is well-defined. It is easy to check that $e$ is a metric on $\mathrm{Hom}_{\mathsf{Top}}(X,Y)$.
	\begin{itemize}
		\item \emph{Non-negativity}. For all $f,g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)$, we have $e(f,g) \ge 0$, and 
		\[
		e(f,g)=0 \iff  d\left(f(x),g(x)\right)=0,\; \forall x \in X\iff f(x)=g(x),\; \forall x \in X\iff f=g.
		\]
		\item \emph{Symmetry}.
		\[
		e(f,g)=\sup_{x\in X}d\left(f(x),g(x)\right)=\sup_{x\in X}d\left(g(x),f(x)\right)=e(g,f).
		\]
		\item \emph{Triangle Inequality}. Since given any $f,g,h\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)$, 
		\[
		 d(f(x),h(x))\le d(f(x),g(x))+d(g(x),h(x)), \quad \forall x \in X,
		\]
		take the supremum and we obtain
		\begin{align*}
			e(f,h)&=\sup_{x\in X}d\left(f(x),h(x)\right)\\
			&\le \sup_{x\in X}\left(d\left(f(x),g(x)\right)+d\left(g(x),h(x)\right)\right)\\
			&\le \sup_{x\in X}d\left(f(x),g(x)\right)+ \sup_{x\in X}d\left(g(x),h(x)\right)\\
			&= e(f,g)+e(g,h).
		\end{align*}
	\end{itemize}
	Let $\tau_{\mathrm{met}}$ be the topology induced by $e$ and $\tau_{\mathrm{co}}$ be the compact-open topology on $\mathrm{Hom}_{\mathsf{Top}}(X,Y)$. We will show that $\tau_{\mathrm{met}}=\tau_{\mathrm{co}}$.
	\begin{enumerate}[(i)]
		\item $\tau_{\mathrm{co}}\subseteq \tau_{\mathrm{met}}$. We need to show that for any compact subset $K\subseteq X$ and any open set $V\subseteq Y$, 
		\[	
		S(K,V)=\left\{f\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\mid f(K)\subseteq V\right\} \in \tau_{\mathrm{met}}.
		\]
        It is sufficient to show that for any $f\in S(K,V)$, there exists an $\epsilon>0$ such that 
		\[
		B(f,\epsilon):=\left\{ g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\midv \sup_{x\in X}d\left(f(x),g(x)\right)<\epsilon \right\} \subseteq S(K,V).
		\]
		Now given any $f\in S(K,V)$, according to \Cref{th:properties_of_distance_to_set}, we can define a continuous function on the compact set $f(K)$
		\begin{align*}
			\phi: f(K) &\longrightarrow \mathbb{R}_{\ge 0} \\
			y&\longmapsto d(y, Y-V)
		\end{align*}
		By \Cref{th:properties_of_distance_to_set}, since $(Y-V)$ is closed, we have $\phi(y)=0\iff y\in Y-V$. Note $f(K)\cap(Y-V)=\varnothing$. There must be $\phi(y)>0$ for any $y\in f(K)$. Thus, $\phi$ has a positive minimum
	    \[
		 \rho:= \min_{y\in f(K)}\phi(y)>0.
		\]
		Let $\epsilon=\frac{\rho}{2}$. For any $g\in B(f,\epsilon)$ and any $x\in K$, we have
		\begin{align*}
			d(g(x),Y-V) \ge d(f(x), Y-V) -d(g(x),f(x))=\phi(f(x)) -d(g(x),f(x))\ge \rho - \epsilon = \frac{\rho}{2}>0.
		\end{align*}
		This implies $g(x)\notin Y-V$, i.e. $g(x)\in V$. Therefore, we conclude that $g(K)\subseteq V$ and $g\in S(K,V)$. Hence, we have $B(f,\epsilon)\subseteq S(K,V)$, proving $\tau_{\mathrm{co}}\subseteq \tau_{\mathrm{met}}$.
		\item $\tau_{\mathrm{met}}\subseteq \tau_{\mathrm{co}}$. By \Cref{th:comparison_of_the_topologies}, it is sufficient to show that for any $f\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)$ and any $\varepsilon>0$, there is a neighborhood $U$ of $f$ in $\tau_{\mathrm{co}}$ such that $U \subseteq B(f,\varepsilon)$. Because $f(X)$ is compact in $Y$, the open cover $\{B(y, \varepsilon / 3)\}_{y \in g(X)}$ of $f(X)$ has a finite subcover
		$$
		f(X) \subseteq \bigcup_{i=1}^n B\left(y_i, \varepsilon / 3\right) \quad\left(y_i \in f(X)\right).
		$$
		Define compact subsets of $X$
		$$
		K_i=\overline{f^{-1}\left(B\left(y_i, \varepsilon / 3\right)\right)} \subseteq X
		$$
		and open sets in $Y$
		$$
		V_i=B\left(y_i, 2 \varepsilon / 3\right) \subseteq Y.
		$$
		Then by \Cref{th:continuous_map_equivalent_definition}, for each $i=1,2,\cdots,n$, we have
		\[
		f\left(K_i\right) \subseteq \overline{f\left(f^{-1}\left(B\left(y_i, \varepsilon / 3\right)\right)\right)} \subseteq\overline{B\left(y_i, \varepsilon / 3\right)} \subseteq V_i,
		\] 
		which implies
		\[
		f \in \bigcap_{i=1}^n S(K_i, V_i).
		\]
		Let $U=\bigcap_{i=1}^n S(K_i, V_i)$, then $U$ is a neighborhood of $f$ in $\tau_{\mathrm{co}}$. Now take any $g \in U$. For a given $x \in X$, choose $i$ with $f(x) \in B\left(y_i, \varepsilon / 3\right)$. Then $x \in f^{-1}\left(B\left(y_i, \varepsilon / 3\right)\right) \subseteq K_i$, hence by $g\in S(K_i,V_i)$ we get $g(x) \in V_i=B\left(y_i, 2 \varepsilon / 3\right)$. Therefore

		$$
		d(g(x), f(x)) \leq d\left(g(x), y_i\right)+d\left(y_i, f(x)\right)<\frac{2 \varepsilon}{3}+\frac{\varepsilon}{3}=\varepsilon .
		$$
        Taking the supremum over $x \in X$ yields $e(g,f)<\varepsilon$, which means $g \in B(f,\varepsilon)$. Thus, we have $U \subseteq B(f,\varepsilon)$, proving $\tau_{\mathrm{met}}\subseteq \tau_{\mathrm{co}}$.
	\end{enumerate}
	Therefore, we conclude that $\tau_{\mathrm{met}}=\tau_{\mathrm{co}}$.
\end{proof}
...
\begin{definition}{Compact Convergence}{}
	Let $X$ be a topological space and $(Y,d)$ be a metric space. A sequence of maps $\left(f_n:X\to Y\right)_{n \ge 1}$ is said to \textbf{converge compactly} to a map $f:X\to Y$ if for every compact subset $K$ of $X$, $\left(f_n|_K\right)_{n \ge 1}$ converges uniformly to $f|_K$, i.e.
	\[
	  \lim_{n\to \infty}\sup_{x\in K}d\left(f_n(x),f(x)\right)=0.
	\]
\end{definition}

\begin{definition}{Topology of Compact Convergence}{}
	Let $X$ be a topological space and $(Y,d)$ be a metric space. The \textbf{topology of compact convergence} on $\mathrm{Hom}_{\mathsf{Top}}(X,Y)$ is the topology defined as follows: for any $f\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)$,
	 \[
	 \mathcal{N}_f=\left\{B(f;K,\epsilon)\midv K\subseteq X\text{ compact},\;\epsilon>0	\right\}
	 \]
	 is a neighborhood subbasis of $f$, where
	 \[
	 B(f;K,\epsilon)=\left\{g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\midv \sup_{x\in K}d\left(g(x),f(x)\right)<\epsilon\right\}.
	 \]
\end{definition}

\begin{proposition}{Equivalent Characterisation of Topology of Compact Convergence}{}
	Let $X$ be a topological space and $(Y,d)$ be a metric space. The following topologies on $\mathrm{Hom}_{\mathsf{Top}}(X,Y)$ coincide:
	\begin{enumerate}[(i)]
		\item The compact-open topology $\tau_{\mathrm{co}}$.
		\item The topology of compact convergence $\tau_{\mathrm{cc}}$.
		\item The initial topology with respect to the maps
	\begin{align*}
		\mathrm{res}_K: \mathrm{Hom}_{\mathsf{Top}}(X,Y) &\longrightarrow \mathrm{Hom}_{\mathsf{Top}}(K,Y)\\
		f &\longmapsto f|_K
	\end{align*}
	for all compact subsets $K \subseteq X$, where $\mathrm{Hom}_{\mathsf{Top}}(K,Y)$ is equipped with the compact-open topology, i.e. the topology induced by the metric
	\begin{align*}
		e_K: \mathrm{Hom}_{\mathsf{Top}}(K,Y)\times\mathrm{Hom}_{\mathsf{Top}}(K,Y) &\longrightarrow \mathbb{R}_{\ge 0} \\
		(f,g) &\longmapsto \sup_{x \in K} d\left(f(x),g(x)\right)
	\end{align*}
	\end{enumerate}
\end{proposition}
\begin{proof}
	\begin{itemize}
		\item $\tau_{\mathrm{co}}\subseteq \tau_{\mathrm{cc}}$. We need to show that for any compact subset $K\subseteq X$ and any open set $V\subseteq Y$, 
		\[	
		S(K,V)=\left\{f\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\mid f(K)\subseteq V\right\} \in \tau_{\mathrm{cc}}.
		\]
        It is sufficient to show that for any $f\in S(K,V)$, there exists an $\epsilon>0$ such that 
		\[
		B(f;K,\epsilon):=\left\{ g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\midv \sup_{x\in K}d\left(f(x),g(x)\right)<\epsilon \right\} \subseteq S(K,V).
		\]
		Fix compact subset $K\subseteq X$ and open set $V\subseteq Y$. If $f\in S(K,V)$, then $f(K)\subseteq V$. For each $y\in f(K)$, choose $\epsilon_y>0$ such that $B(y,2 \epsilon_y)\subseteq V$. Because $f(K)$ is compact in $Y$, the open cover $\{B(y, \epsilon_y)\}_{y \in f(K)}$ of $f(K)$ has a finite subcover
		$$
		f(X) \subseteq \bigcup_{i=1}^n B\left(y_i, \epsilon_{y_i}\right) \quad\left(y_i \in f(K)\right).
		$$
		Let $\epsilon=\min\limits_{1\le i\le n}\epsilon_{y_i}>0$. For any $g\in B(f;K,\epsilon)$, we have
		\[
		d(g(x),f(x))<\epsilon,\quad\forall x \in K.
		\]
		 For each $x\in K$, choose $j(x)\in \{1,2,\cdots,n\}$ such that $f(x)\in B(y_{j(x)},\epsilon_{y_{j(x)}})$. Then for any $x\in K$, we have
		$$
		d\left(g(x),y_{j(x)}\right)\le d(g(x), f(x))+ d\left(f(x), y_{j(x)}\right)<\epsilon+\epsilon_{y_{j(x)}}\le 2\epsilon_{y_{j(x)}},
		$$
		which implies $g(x)\in B\left(y_{j(x)}, 2\epsilon_{y_{j(x)}}\right)\subseteq V$. Thus, $g(K)\subseteq V$, proving $B(f;K,\epsilon)\subseteq S(K,V)$. Therefore, we conclude $\tau_{\mathrm{co}}\subseteq \tau_{\mathrm{cc}}$.
		\item $\tau_{\mathrm{cc}}\subseteq \tau_{\mathrm{init}}$. has a subbasis of the form $B(f;K,\epsilon)$, where $f\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)$, $K\subseteq X$ is compact, and $\epsilon>0$. It suffices to show that for each subbasic set $B(f;K,\epsilon)$, there exists an open set $U$ of $\mathrm{Hom}_{\mathsf{Top}}(K,Y)$ such that 
		$$
           B(f;K,\epsilon)=\mathrm{res}_K^{-1}\left(U\right).
		$$
		Let
		\[
		U=B_K(f|_K,\epsilon):=\Bigl\{\varphi\in \mathrm{Hom}_{\mathsf{Top}}(K,Y): e_K(\varphi,f|_K)<\epsilon\Bigr\}
		\]
		denote the open ball in $\mathrm{Hom}_{\mathsf{Top}}(K,Y)$ which is centered at $f|_K$ with radius $\epsilon>0$. Then we can check
		\begin{align*}
			\mathrm{res}_K^{-1}\left(U\right)&=\left\{g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\midv \mathrm{res}_K(g)\in B_K(f|_K,\epsilon)\right\}\\
			&=\left\{g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\midv g|_K\in B_K(f|_K,\epsilon)\right\}\\
			&=\left\{g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\midv e_K(g|_K,f|_K)<\epsilon\right\}\\
			&=\left\{g\in \mathrm{Hom}_{\mathsf{Top}}(X,Y)\midv \sup_{x\in K}d\left(g|_K,f|_K\right)<\epsilon\right\}\\
			&=B(f;K,\epsilon).
		\end{align*}
...
\begin{proposition}{Categorial Properties of $\mathsf{Top}$}{}
	\begin{enumerate}[(i)]
...
\begin{proposition}{Equivalent Characterisation of Initial Topology}{}
...
\end{proposition}
\begin{prf}
	(i)$\implies$(ii). If $\tau$ is the initial topology on $X$ with respect to $\left(f_i\right)_{i \in I}$, then for each $i\in I$, $f_i$ is continuous. Therefore, if $x_\alpha\rightarrow x$ in $(X,\tau)$, then for each $i\in I$, $f_i(x_\alpha)\rightarrow f_i(x)$ in $Y_i$. Conversely, if for each $i\in I$, $f_i(x_\alpha)\rightarrow f_i(x)$ in $Y_i$, then for any open set $U\in \tau$, there exists an index $i\in I$ such that $f_i^{-1}(U)$ is open in $(X,\tau)$. Since $f_i(x_\alpha)\rightarrow f_i(x)$, there exists $\alpha_0\in A$ such that for all $\alpha\geq \alpha_0$, $f_i(x_\alpha)\in U$. Hence, $x_\alpha\rightarrow x$ in $(X,\tau)$.\\

	(ii)$\implies$(i). Omitted.
\end{prf}
...
\begin{proposition}{Limit Points and Subspaces}{limit_points_and_subspaces}
Let \(X\) be a topological space, let \(A\subseteq X\) be equipped with the subspace topology, and let
\(F\subseteq A\). For every \(x\in A\), the following are equivalent:
\[
    x \text{ is a limit point of } F \text{ in } A
    \quad\Longleftrightarrow\quad
    x \text{ is a limit point of } F \text{ in } X.
\]
\end{proposition}

\begin{prf}
\((\implies)\) Assume that \(x\) is a limit point of \(F\) in \(A\). Let \(N\) be a neighborhood of \(x\) in \(X\).
Since \(x\in A\), $N\cap A$ is a neighborhood of \(x\) in the subspace \(A\). Hence, by assumption,
\[
    (N\cap A)\cap (F-\{x\})\neq\varnothing.
\]
Because \(F\subseteq A\), we have \(F-\{x\}\subseteq A\)l, which implies
\[
    (N\cap A)\cap (F-\{x\})
    =
    N\cap (F-\{x\}).
...
\[
    N\cap (F-\{x\})\neq\varnothing.
\]
So \(x\) is a limit point of \(F\) in \(X\).

\((\Longleftarrow)\) Assume that \(x\) is a limit point of \(F\) in \(X\). Let \(U\) be a neighborhood of \(x\) in the subspace \(A\).
By definition of the subspace topology, there exists a neighborhood \(N\) of \(x\) in \(X\) such that
\[
    N\cap A\subseteq U.
\]
Since \(x\) is a limit point of \(F\) in \(X\), we have
\[
    N\cap (F-\{x\})\neq\varnothing.
\]
But \((F-\{x\})\subseteq A\), so
\[
    N\cap (F-\{x\})
    =
    (N\cap A)\cap (F-\{x\})
...
\[
    U\cap (F-\{x\})\neq\varnothing.
\]
So \(x\) is a limit point of \(F\) in \(A\).
...
\begin{proposition}{Closure of Product of Subsets}{closure_of_product_of_subsets}
Let $(X_i)_{i\in I}$ be a family of topological spaces, and let
$A_i\subseteq X_i$ for each $i\in I$. Equip $\prod_{i\in I}X_i$ with the
product topology. Then
\[
\overline{\prod_{i\in I} A_i}
=
\prod_{i\in I}\overline{A_i},
\]
where the closure on the left is taken in $\prod_{i\in I}X_i$, and the
closure $\overline{A_i}$ is taken in $X_i$.
\end{proposition}

\begin{prf}
First, since $A_i\subseteq \overline{A_i}$ for each $i\in I$, we have
\[
\prod_{i\in I}A_i \subseteq \prod_{i\in I}\overline{A_i}.
\]
Moreover, $\prod_{i\in I}\overline{A_i}$ is closed in $\prod_{i\in I}X_i$,
because each $\overline{A_i}$ is closed in $X_i$, and arbitrary products of
closed sets are closed in the product topology. Hence
\[
\overline{\prod_{i\in I}A_i}
\subseteq
\prod_{i\in I}\overline{A_i}.
...
Conversely, let
\[
x=(x_i)_{i\in I}\in \prod_{i\in I}\overline{A_i}.
\]
We show that $x\in \overline{\prod_{i\in I}A_i}$. Let $U$ be an open
neighbourhood of $x$ in $\prod_{i\in I}X_i$. Since the product topology has
a basis consisting of sets of the form
\[
\prod_{i\in I}U_i,
\]
where each $U_i\subseteq X_i$ is open and $U_i=X_i$ for all but finitely many
$i$, there exists a basic open set $\prod_{i\in I}U_i$
such that
\[
x\in \prod_{i\in I}U_i \subseteq U.
\]
Let
\[
F=\{i\in I : U_i\neq X_i\}.
\]
Then $F$ is finite. For each $i\in F$, we have $x_i\in U_i$ and
$x_i\in \overline{A_i}$, which implies $U_i\cap A_i\neq \varnothing$. Hence, for each $i\in F$, we can choose $a_i\in U_i\cap A_i$. For each $i\in I-F$, we can choose $a_i\in A_i=U_i\cap A_i$. Then we have
\[
a=(a_i)_{i\in I}\in \prod_{i\in I}A_i
\]
and also
\[
a\in \prod_{i\in I}U_i \subseteq U.
\]
Thus every open neighbourhood $U$ of $x$ intersects
$\prod_{i\in I}A_i$, so $x\in \overline{\prod_{i\in I}A_i}$. Therefore
\[
\prod_{i\in I}\overline{A_i}
\subseteq
\overline{\prod_{i\in I}A_i}.
...
\begin{proposition}{Properties of $\mathrm{T}_1$ Space}{properties_of_t1_space}
	Let $X$ be a $\mathrm{T}_1$ space. Then
	\begin{enumerate}[(i)]
		\item Subspaces and products of $\mathrm{T}_1$ spaces are $\mathrm{T}_1$.
		\item Suppose \(A\subseteq X\). If \(x\in X\) is a \hyperref[th:limit_point]{limit point} of \(A\), i.e. $x\in A'$, then every neighborhood of \(x\) in $X$ contains infinitely many points of \(A\).
	\end{enumerate}
\end{proposition}
\begin{prf}
	\begin{enumerate}[(i)]
		\item Suppose $X$ is a $\mathrm{T}_1$ space and $Y$ is a subspace of $X$. Then for any $y\in Y$, we have
		\[
		\mathrm{cl}_X(\{y\})=\{y\},
		\]
		which implies that
		\[
        \mathrm{cl}_Y(\{y\})=\mathrm{cl}_X(\{y\})\cap Y=\{y\}\cap Y=\{y\}.
		\]
		Thus
		$\{y\}=\{y\}\cap Y$ is closed in $Y$. 
		
		If $\{X_i\}_{i\in I}$ is a family of $\mathrm{T}_1$ spaces, then for any $(x_i)_{i\in I}\in \prod_{i\in I}X_i$, by \Cref{th:closure_of_product_of_subsets} we have
        \[
		 \overline{\{(x_i)_{i\in I}\}}=\overline{\prod_{i\in I}\{x_i\}}=\prod_{i\in I}\overline{\{x_i\}}=\prod_{i\in I}\{x_i\}=\{(x_i)_{i\in I}\},
		\]
		which implies that $\{(x_i)_{i\in I}\}$ is closed in $\prod_{i\in I}X_i$. Hence $\prod_{i\in I}X_i$ is a $\mathrm{T}_1$ space.

		\item 
		Suppose there exists a neighborhood \(N\) of \(x\) in \(X\) such
		that \(N\cap A\) is finite. Since \(X\) is \(T_1\), every finite subset of \(X\) is
		closed by \Cref{th:equivalent_definitions_of_t1_space}. Therefore
		\[
			F:=(N\cap A)-\{x\}
		\]
		is closed in \(X\). Thus $X-F$ is an open neighborhood of \(x\) in \(X\). Hence
		\[
			N\cap (X - F)
		\]
		is a neighborhood of \(x\) in \(X\). On the one hand, we have
		\[
		\begin{aligned}
		\left(N\cap (X-F)\right)\cap (A-\{x\})
		&=N\cap (X-F)\cap A\cap (X-\{x\})\\
		&=(N\cap A\cap (X-\{x\}))\cap (X-F)\\
		&=((N\cap A)-\{x\})\cap (X-F)\\
		&=F\cap (X-F)\\
		&=\varnothing.
		\end{aligned}
		\]
		On the other hand, since \(x\) is a limit point of \(A\) in
		\(X\), and \(N\cap (X-F)\) is a neighborhood of \(x\) in \(X\), we have
		\[
			\left(N\cap (X-F)\right)\cap (A-\{x\})\ne \varnothing,
		\]
		which is a contradiction. Therefore, for every neighborhood \(N\) of \(x\) in \(X\), \(N\cap A\) is infinite.
	\end{enumerate}
\end{prf}
...
\begin{proposition}{Properties of Hausdorff Space}{properties_of_hausdorff_space}
	...
	\begin{enumerate}[(i)]
		...
		\item Suppose $x\in X$ and $Y$ is a compact subset of $X$ such that $x\notin Y$. Then there exist neighborhoods $U$ of $x$ and $V$ of $Y$ such that $U\cap V=\varnothing$.
	\end{enumerate}
\end{proposition}
\begin{prf}
	\begin{enumerate}[(i)]
...
\begin{proposition}{Sujective Open Maps Preserve Second-Countability}{}
	If $f:X\to Y$ is a surjective open map and $X$ is second-countable, then $Y$ is second-countable.
\end{proposition}
\begin{prf}
	Let $\mathcal{B}$ be a countable base of $X$. We show that $\{f(B)\mid B\in \mathcal{B}\}$ is a countable base of $Y$. Since $f$ is surjective, for any $y\in Y$, there exists $x\in X$ such that $f(x)=y$. Since $\mathcal{B}$ is a base of $X$, for any neighborhood $N$ of $x$ in $X$, there exists $B_N\in \mathcal{B}$ such that $x\in B_N\subseteq N$. Since $f$ is an open map, $f(B_N)$ is an open neighborhood of $y$ in $Y$. Moreover, since $f(B_N)\subseteq f(N)$, we have
	\[
		y=f(x)\in f(B_N)\subseteq f(N).
	\]
	Hence $\{f(B)\mid B\in \mathcal{B}\}$ is a base of $Y$. Since $\mathcal{B}$ is countable, $\{f(B)\mid B\in \mathcal{B}\}$ is countable. Therefore, $Y$ is second-countable.
\end{prf}
...
\begin{definition}{Compact Subset}{}
	...
	Here, an open cover of $K$ in $X$ is a collection $\{U_\alpha\}_{\alpha\in I}$ of open subsets of $X$ such that $K\subseteq \bigcup_{\alpha\in I}U_\alpha$. 
\end{definition}
...
\begin{proposition}{Properties of Compact Space}{properties_of_compact_space}
	Let $X$ be a compact topological space. Then
	\begin{enumerate}[(i)]
		\item If \(A\subseteq X\) is infinite, then \(A\) has at least one limit point in \(X\).
	\end{enumerate}
\end{proposition}
\begin{prf}
	\begin{enumerate}[(i)]
		\item  Suppose, for contradiction, that \(A\) has no limit point in \(X\). Then $A'=\varnothing \subseteq A$. By \Cref{th:equivalent_characterisations_of_closed_set}, $A$ is closed in $X$. By \Cref{th:sufficient_condition_for_compactness}, $A$ is compact.
		
		For every \(x\in X\), since \(x\) is not a limit point of \(A\), there exists an open neighborhood \(U_x\) of \(x\) in \(X\) such that
	\[
		U_x\cap (A-\{x\})=\varnothing.
	\]
	Note $U_x\cap (A-\{x\}) =(U_x\cap A) - \{x\}$. This means 
	\[
		U_x\cap A\subseteq \{x\}.
	\]
	Note that \(\{U_x\}_{x\in A}\) is an open cover of \(A\), there exist a finite number of points $x_1, \cdots, x_n\in A$ such that
	\[
		A\subseteq \bigcup_{i=1}^n U_{x_i}.
	\]
    Thus
	\[
	   A\subseteq A \cap \bigcup_{i=1}^n U_{x_i} = \bigcup_{i=1}^n (A\cap U_{x_i}) \subseteq \bigcup_{i=1}^n \{x_i\} = \{x_1, \cdots, x_n\},
	\]
	which contradicts the assumption that $A$ is infinite. Therefore, \(A\) has at least one limit point in \(X\).
	\end{enumerate}
\end{prf}
...
\begin{definition}{}{}
	...
	\]
\end{definition}
\begin{proposition}{}{properties_of_distance_to_set}
...
\begin{lemma}{}{shrink_the_neighborhood_of_ab}
	Let $G$ be a topological group. The map 
	\begin{align*}
	...
	\begin{enumerate}[(i)]
		...
		\]
		\item For any neighborhood $W$ of $1_G$, there exists an open neighborhood $U$ of $1_G$ such that 
		\[
		m(U\times U^{-1})=U U\subseteq W.
		\]
		\item For any neighborhood $W$ of $1_G$, there exists an open neighborhood $U$ of $1_G$ such that 
		\[
		U^{(n)}=\left\{ g_1 g_2 \cdots g_n \mid g_i\in U \text{ for } i=1,2,\ldots,n\right\} \subseteq W.
		\]
	\end{enumerate}
\end{lemma}
\begin{prf}
	...
	\item Take $a=b=1_G$ in (i). For any neighborhood $W$ of $1_G$, there exists an open neighborhood $U_1$ of $1_G$ and an open neighborhood $U_2$ of $1_G$ such that $m(U_1\times U_2)\subseteq W$. Let $U=U_1\cap U_2^{-1}$ and $\mathrm{inv}:g\mapsto g^{-1}$. Then we have 
	\[
	U^{-1}=\mathrm{inv}(U_1\cap \mathrm{inv}^{-1}(U_2))=\mathrm{inv}(U_1)\cap U_2\subseteq U_2
	\] 
	and $UU=U(U^{-1})^{-1}=  m(U\times U^{-1})\subseteq m(U_1\times U_2)\subseteq W$.
	\item We can prove this by induction on $n$. For $n=2$, it is just (iii). Suppose it holds for $n=k$. From (iii), there exists an open neighborhood $V$ of $1_G$ such that $VV\subseteq W$. By the induction hypothesis, there exists an open neighborhood $U_0$ of $1_G$ such that $U_0^{(k)}\subseteq V$. Let $U:=U_0\cap V$. Then $U$ is an open neighborhood of $1_G$ such that
	\[
		U^{(k+1)}=U^{(k)}U\subseteq U_0^{(k)} V\subseteq VV\subseteq W.
	\]
...
\begin{lemma}{Subgroup $G_S$ of the Restricted Product}{subgroup_G_S_of_the_restricted_product}
	...
	\]
	Endow $G_S$ with the subspace topology from $G$. Then the following hold:
	\begin{enumerate}[(i)]
		\item The subspace topology $\tau_{\mathrm{sub}}$ on $G_S$ coincides with the product topology $\tau_{\mathrm{prod}}$ on $G_S$.
		\item $G_S$ is a locally compact group.
	\end{enumerate}
\end{lemma}
\begin{prf}
	\begin{enumerate}[(i)]
		\item First, we prove $\tau_{\mathrm{sub}} \subseteq \tau_{\mathrm{prod}}$. Note that $\tau_{\mathrm{sub}}$ has basis of open sets of the form
	\[	
	...
	\item By \hyperref[th:sufficient_condition_for_compactness]{Tychonoff's theorem}, the product of compact spaces is compact. So 
	\[
	 \prod_{i\in I-S} K_i
	\]
	is compact. According to \Cref{th:properties_of_locally_compact_space}, since $G_S$ is a product of a finite number of locally compact groups, $G_S$ is locally compact.
	\end{enumerate}
	
\end{prf}

\begin{proposition}{Restricted Product as a Direct Limit}{restricted_product_as_direct_limit}
	Let \( I \) be an index set and \( F\subseteq I \) be a finite subset of \( I \). Suppose \( G_i \) is a locally compact group for each \( i \in I \), and \( K_i \subseteq G_i \) is an open compact subgroup for each \( i \in I - F \). Let
	\[
	G:=\prod_{i\in I}' G_i
	\]
	...
	\[
		G_S:=\prod_{i\in S} G_i \times \prod_{i\in I-S} K_i.
	\]
	Endow $G_S$ with the subspace topology from $G$. Then the family $\left( G_S \right)_{S\in \mathcal S}$ together with
the inclusion maps 
    \[
...
\begin{prf}
	...
	\smallskip
	\noindent\textbf{2. The topology $\tau$ on $G$ is the final topology $\tau_{\mathrm{fin}}$ for the family
	...
	Then we prove $\tau_{\mathrm{fin}}\subseteq \tau$. If $U \in \tau_{\mathrm{fin}}$, then for each $S \in \mathcal{S}$, $i^{-1}_S(U)=U\cap G_S$ is open in $G_S$. Given any $g\in U$, we have $g\in G_{S_g}$. So $U\cap G_{S_g}$ is a neighborhood of $g$ in $G_S$. According to \Cref{th:subgroup_G_S_of_the_restricted_product}, the topology on $G_S$ is the product topology, which implies that $G_S$ has a neighbor basis at the point $g$ of all sets of the form
	\[
	\prod_{i\in I} N_i,
	\]
	...
	\smallskip
\noindent\textbf{3. Universal property.}\\
...
\begin{itemize}
	\item \emph{Definition of the candidate map.}
	Define a map
	\begin{align*}
...
\begin{prf}
	\begin{enumerate}[(i)]
		\item Given any $g\in G$, define 
	\[
	S_g\;:=\;F\;\cup\;\left\{\,i\in I- F \mid g_i\notin K_i\right\}.
	\]
	From \Cref{th:restricted_product_as_direct_limit} we see $G_{S_g}$ is a locally compact group and $g\in G_{S_g}$. Thus  $G_{S_g}$ is a compact neighborhood of $g$ in $G$. This shows that $G$ is locally compact.
	\end{enumerate}
\end{prf}
...
\begin{proposition}{Hausdorff Criterion for Orbit Spaces}{hausdorff_criterion_for_orbit_spaces}
...
\end{proposition}
\begin{prf}
	This proposition is a corollary of \Cref{th:criterion_for_Hausdorffness_of_quotient_space}. Here we give a direct proof.
...
\begin{proposition}{Equivalent Definition of Proper Action on Hausdorff Space}{equivalent_definition_of_proper_action_on_Hausdorff_space}
...
\begin{proposition}{}{}
	If a topological group $G$ acts continuously and properly on a locally compact Hausdorff space $E$, then the orbit space $G\backslash E$ is Hausdorff.
\end{proposition}
\begin{prf}
...
\begin{proposition}{Covering Space over a Hausdorff Space is Hausdorff}{covering_space_over_Hausdorff}
	Let \(p:X\to Y\) be a covering map. If \(Y\) is Hausdorff, then \(X\) is Hausdorff.
\end{proposition}

\begin{prf}
	Let \(x_1,x_2\in X\) with \(x_1\neq x_2\). We show that they have disjoint open neighborhoods.

	First suppose \(p(x_1)\neq p(x_2)\). Since \(Y\) is Hausdorff, there exist disjoint open neighborhoods
	\[
		U_1\ni p(x_1),\qquad U_2\ni p(x_2).
	\]
	Then
	\[
		p^{-1}(U_1),\qquad p^{-1}(U_2)
	\]
	are disjoint open neighborhoods of \(x_1\) and \(x_2\), respectively.

	Now suppose \(p(x_1)=p(x_2)=y\). Since \(p\) is a covering map, there exists an evenly covered open neighborhood
	\(U\subseteq Y\) of \(y\). Thus
	\[
		p^{-1}(U)=\coprod_{\alpha\in A} V_\alpha,
	\]
	where each \(V_\alpha\subseteq X\) is open and $p|_{V_\alpha}:V_\alpha\to U$ is a homeomorphism.

	Because \(p|_{V_\alpha}\) is injective, two distinct points of the same fiber cannot lie in the same sheet \(V_\alpha\). Hence \(x_1\) and \(x_2\) lie in two distinct sheets, say
	\[
		x_1\in V_\alpha,\qquad x_2\in V_\beta,\qquad \alpha\neq\beta.
	\]
	But the sheets are pairwise disjoint, so \(V_\alpha\) and \(V_\beta\) are disjoint open neighborhoods of \(x_1\) and \(x_2\). Therefore, \(X\) is Hausdorff.
\end{prf}
...
\begin{proposition}{Even Actions Force the Acting Group to be Discrete}{even_action_forces_discrete_group}
	Let \(G\) be a topological group acting continuously on a nonempty topological space \(X\). Suppose that the action is even. Then \(G\) is discrete.
\end{proposition}
\begin{proof}
	Since \(X\neq\varnothing\), choose a point \(x\in X\). By evenness, we can choose an open neighborhood \(U\ni x\) such that
	\[
		gU\cap U\neq\varnothing \implies g=1_G.
	\]
	...
	Since the action is continuous, \(\alpha_x\) is continuous. Because \(U\) is an
	open neighborhood of \(x=\alpha_x(1_G)\), there exists an open neighborhood
	\(W\) of \(1_G\) in \(G\) such that
	\[
		\alpha_x(W)=Wx\subseteq U.
	\]

	We claim that \(W=\{1_G\}\). Indeed, given any \(g\in W\), we have \(gx\in U\). On the other hand, since \(x\in U\), we also have $gx\in gU$. Therefore
	\[
		gU\cap U\neq\varnothing\implies g=1_G.
	\]
	So we have \(W=\{1_G\}\). Thus we see \(\{1_G\}\) is open in \(G\). Since left translation by any \(g\in G\) is a homeomorphism, every singleton
	\[
		\{g\}=g\{1_G\}
	\]
	is also open. Therefore, \(G\) is discrete.
\end{proof}
...
\begin{definition}{Hausdorff Even Action}{}
Let $G$ be a topological group acting continuously on a topological space $E$. We say that the action is \textbf{Hausdorff even} if the following two conditions hold:
\begin{enumerate}[(i)]
    \item The action is even.
    \item If $x,x'\in E$ are not in the same $G$-orbit, then there exist open neighborhoods
    $U\ni x$ and $U'\ni x'$ such that
    \[
    gU\cap U'=\varnothing
...
\end{enumerate}
\end{definition}
In some literature, Hausdorff even actions are also called \textbf{properly discontinuous} actions.

\begin{lemma}{}{even_action_with_closed_orbit_relation_is_compactly_finite}
Let a discrete group \(\Gamma\) act evenly on a $\mathrm{T}_1$ space \(E\). Suppose that the orbit relation
\[
    \mathcal O
    =
    \{(ge,e)\in E\times E \mid g\in \Gamma,\ e\in E\}
\]
is closed in \(E\times E\). Then for every compact subset \(K\subseteq E\), the set
\[
    \Gamma_K
    =
    \{g\in \Gamma \mid gK\cap K\neq\varnothing\}
\]
is finite.
\end{lemma}

\begin{prf}
Suppose, for contradiction, that \(\Gamma_K\) is infinite. For each \(g\in \Gamma_K\), since $gK\cap K\neq \varnothing$, we can choose a point \(x_g\in gK\cap K\). And we can choose a point \(e_g\in K\) such that
    \[
        x_g=g e_g.
    \]
    Define a map
	\begin{align*}
		 F:\Gamma_K & \longrightarrow K\times K \\
		 g        & \longmapsto (ge_g,e_g)
	\end{align*}
	We first claim that \(F\) is injective. Indeed, suppose $F(g)=F(h)$ for some \(g,h\in \Gamma_K\). Then
    \[
        ge_g=he_h
        \qquad\text{and}\qquad
        e_g=e_h.
    \]
    Since an even action is free, it follows that \(g=h\). Thus \(F\) is injective. Consequently,
    \[
    	A:=F(\Gamma_K)=\{(ge_g,e_g)\mid g\in \Gamma_K\}
	\]
	is an infinite subset of the compact space \(K\times K\). By \Cref{th:properties_of_compact_space}, every infinite subset of a compact space has a limit point. Therefore \(A\) has a limit point \((x_0,y_0)\in K\times K\). Since
\(A\subseteq \mathcal O\) and \(\mathcal O\) is closed, we have
\((x_0,y_0)\in \mathcal O\). Thus there exists \(g_0\in \Gamma\) such that
\[
    x_0=g_0y_0.
\]
Choose an even neighborhood \(U\) of \(y_0\). Then \(g_0U\times U\) is a
neighborhood of \((x_0,y_0)\) in \(E\times E\). By \Cref{th:limit_points_and_subspaces}, in the space $E\times E$, $(x_0, y_0)$ is also a limit point of the subset $A\subseteq E\times E$. From \Cref{th:properties_of_t1_space} we know that $E\times E$ is a $\mathrm{T}_1$ space. According to \Cref{th:properties_of_t1_space}, since $A$ is a subset of the $\mathrm{T}_1$ space $E\times E$ and $A$ has a limit point $(x_0,y_0)$, every neighborhood of $(x_0,y_0)$ in $E\times E$ contains infinitely many points of $A$. In particular,
\[
\left(g_0U\times U\right) \cap A=\left(g_0U\times U\right) \cap F(\Gamma_K)
\]
is an infinite set. So it must be nonempty and we can assume there exists $z\in \left(g_0U\times U\right) \cap F(\Gamma_K)$. Then there exists a unique $g\in \Gamma_K$ such that $z=F(g)$. Note that
\[
    F(g)=(ge_g,e_g)\in g_0U\times U.
\]
We have \(e_g\in U\) and \(ge_g\in g_0U\), which implies
\[
    gU\cap g_0U\neq\varnothing.
\]
Since \(U\) is even, this forces \(g=g_0\) and accordingly \(z=F(g_0)\). Therefore \(\left(g_0U\times U\right) \cap F(\Gamma_K)\) contains only one element, contradicting the fact that it is an infinite set. Thus \(\Gamma_K\) must be finite.
\end{prf}


\begin{proposition}{Hausdorff Even Action is Proper}{}
Suppose that a discrete topological group \(\Gamma\) acts on a topological space \(E\).
If \(\Gamma \acts E\) is a Hausdorff even action, then:
\begin{enumerate}[(i)]
    \item the orbit space \(\Gamma\backslash E\) is Hausdorff;
    \item \(E\) is Hausdorff;
    \item the action \(\Gamma\acts E\) is proper.
\end{enumerate}
\end{proposition}

\begin{prf}
\begin{enumerate}[(i)]
    \item This is a direct consequence of \Cref{th:hausdorff_criterion_for_orbit_spaces}.

    \item Since the action is even, the quotient map
    \[
        p:E\to \Gamma\backslash E
    \]
    is a covering map. By part (i), \(\Gamma\backslash E\) is Hausdorff.
    Hence \(E\) is Hausdorff by
    \Cref{th:covering_space_over_Hausdorff}.

    \item By
    \Cref{th:equivalent_definition_of_proper_action_on_Hausdorff_space}, it
    suffices to show that for every compact subset \(K\subseteq E\), the set
    \[
        \Gamma_K
        =
        \{g\in \Gamma \mid gK\cap K\neq \varnothing\}
    \]
    is compact. Since \(\Gamma\) is discrete, this is equivalent to showing that \(\Gamma_K\) is finite. Let
    \[
        \mathcal O
        :=
        \{(ge,e)\in E\times E \mid g\in \Gamma,\ e\in E\}
    \]
    be the orbit relation and
	\[
	   \Delta_{\Gamma\backslash E}:=\{(x,x)\in (\Gamma\backslash E)\times (\Gamma\backslash E)\mid x\in \Gamma\backslash E\}
	\]
	be the diagonal in \((\Gamma\backslash E)\times (\Gamma\backslash E)\). Since \(\Gamma\backslash E\) is Hausdorff, the diagonal \(\Delta_{\Gamma\backslash E}\) is closed. Note that 
    \[
        \mathcal O=(p\times p)^{-1}
        \left(\Delta_{\Gamma\backslash E}\right)
    \]
	is the preimage of a closed set under the continuous map \(p\times p: E\times E\to (\Gamma\backslash E)\times (\Gamma\backslash E)\). So \(\mathcal O\) is closed in
    \(E\times E\). According to \Cref{th:even_action_with_closed_orbit_relation_is_compactly_finite}, \(\Gamma_K\) must be finite, which completes the proof that the action is proper.
\end{enumerate}
\end{prf}
