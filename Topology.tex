...
$$
H_n(X)\cong\bigoplus_{\alpha\in \pi_0(X)} H_n\left(X_\alpha\right),
$$ 
where $X_\alpha$ are path-components of $X$. Especially, we have $H_0(X)\cong \mathbb{Z}^{\oplus \pi_0(X)}$. 
\end{proposition}
\begin{proof}
	First we apply the functor $C_k:\mathsf{Top}\to\mathsf{Ab}$ to the inclusions $i_\alpha:X_\alpha \hookrightarrow X$ to get $C_k(i_\alpha)$, that is,
	\begin{align*}
		\left(i_\alpha\right)_*: C_k(X_\alpha)&\longrightarrow C_k(X)\\
	...
	\begin{equation*}
		\begin{tikzcd}		
			& C_{k-1}(X) \\ [10pt]
			C_{k}(X_\alpha) \arrow[ru, "\varphi"] \arrow[r, "\left(i_\alpha \right)_{*}"']
			& \bigoplus\limits_{\alpha\in \pi_0(X)} C_n\left(X_\alpha\right) \arrow[u, "\varphi"'] 
		\end{tikzcd}			
	\end{equation*}
\end{proof}
