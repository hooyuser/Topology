...
\dfn{Topological Space}{
	A \textbf{topological space} is an ordered pair $(X,\tau)$, where $X$ is a set and $\tau\subset\mathcal{P}(X)$ is a collection of subsets of $X$, satisfying the following axioms:
	\begin{enumerate}
...
\dfn{Closed Set}{
	Suppose that $A$ is a subset of $X$. $A$ is a \textbf{closed set} if and only if $A^C$ is an open set.
}
...
\dfn{Neighborhood}{
	Given a point $x\in X$, a set $N\subset X$ is the \textbf{neighborhood} of $x$ if there exists an open set $O\in\tau$ such that $x\in O\subset N$. The collection of all neighborhoods of $x$ is denoted by $\mathcal{N}(x)$.
}
...
\dfn{Locally Connected Topological Space}{
	A topological space $X$ is \textbf{locally connected} if for every $x\in X$ and every open set $U$ containing $x$, there is a connected open set $V$ such that $x\in V\subseteq U$.
}
...
\dfn{Locally Path Connected}{
	A topological space $X$ is \textbf{locally path connected} if for every $x\in X$ and every open set $U$ containing $x$, there is a path connected open set $V$ such that $x\in V\subseteq U$.
}
...
\section{Homotopy}
\dfn{Path}{	
	A \textbf{path} in a topological space $X$ is defined as a continuous function $p:[0,1]\to X$. $p$ is said to be from $x$ to $y$ if $p(0)=x$ and $p(1)=y$.
}
We say a path is a \textbf{loop} if $p(0)=p(1)$.

\dfn{Homotopy}{
	A \textbf{homotopy} between two continuous functions $f$ and $g$ from a topological space $X$ to a topological space $Y$ is defined to be a continuous function $h: X \times[0,1] \rightarrow Y$ such that $h(x, 0)=f(x)$ and $H(x, 1)=g(x)$ for all $x \in X$.
}
...
\dfn{Homotopy Rel End Points}{
	Let $x,y$ be two points in a topological space $X$. If $p:[0,1]\to X$ and $q:[0,1]\to X$ are two paths from $x$ to $y$, then a \textbf{homotopy rel end points from $p$ to $q$} is defined as a homotopy relative to $\{x, y\}$ from $p$ to $q$.
}
\dfn{Equivalent Paths}{
	Let $X$ be a topological space. Suppose $f: [0,1] \longrightarrow X$ is a path from $x$ to $y$ are \textbf{equivalent} if they are homotopic rel end points. That is, there must exist a homotopy $h: [0,1] \times [0,1] \longrightarrow X$ such that
	$$
...
\dfn{Composition Of Paths}{
	Let $x,y,z$ be $3$ points in a topological space $X$. Suppose $f: [0,1] \longrightarrow X$ is a path from $x$ to $y$ and $g: [0,1] \longrightarrow X$ is a path from $y$ to $z$. The \textbf{composition of path $f$ and $g$} denoted by $g\cdot f$ is defined to be a path from $x$ to $z$ as follows
	\begin{align*}
...
\dfn{Composition Of Equivalent Classes Of Paths}{
	Suppose $f: [0,1] \longrightarrow X$ is a path from $x$ to $y$ and $g: [0,1] \longrightarrow X$ is a path from $y$ to $z$. The \textbf{composition of $[f]$ and $[g]$} denoted by $[g]\cdot [f]$ is defined as follows
	\[
		[g]\cdot [f]:=[g\cdot f].
	\]
}
...
\dfn{Inverse Of Path}{
	Let $p: [0,1] \longrightarrow X$ is a path from $x$ to $y$. The \textbf{inverse of path $p$} denoted by $p^{-1}$ is defined to be a path from $y$ to $x$ as follows
	\begin{align*}
...
\dfn{Inverse Of Equivalent Class Of Paths}{
	Let $X$ be a topological space and $p:[0,1]\to X$ be a path. Then the \textbf{inverse of path $p$} denoted by $[p]^{-1}$ is defined as follows
	\begin{align*}
		[p]^{-1}:=[p^{-1}].
	\end{align*}
}
...
\dfn{Subcomplex}{
Let $K$ be a simplicial complex. If $L\subset K$ and $L$ is a simplicial complex, then $L$ is called a \textbf{subcomplex} of $K$.
}
...
\dfn{Standard Simplex}{
	For $n \geq 0$, the \textbf{standard $n$-simplex} $\Delta^n$ is the set $\Delta^n \subset \mathbb{R}^{n+1}$ defined by
$$
